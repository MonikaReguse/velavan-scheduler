...
async function getConfig(businessId: string) {
...
export async function POST(request: Request) {
  try {
    ...
    console.log("Incoming POST payload:", { platforms, mediaUrl, mediaType });
    ...
    // Wait for all selected platforms to finish posting concurrently
    ...
    if (platforms.includes('pinterest')) {
      ...
      if (!pinConfig || !pinConfig.accessToken) {
        ...
        try {
          // First, fetch the user's boards to get a Board ID to post to
          ...
          if (boardsData.items && boardsData.items.length > 0) {
            const boardId = boardsData.items[0].id; // Just pick the first board

            const pinPayload = {
              board_id: boardId,
              title: content.substring(0, 50), // Pinterest requires a title
              description: content,
              media_source: {
      ...
      try {
        ...
        if (successResults.length > 0) {
          ...
          successResults.forEach((r: any) => {
            platformIds[r.platform] = r.id; // e.g. { facebook: 'fb_id', linkedin: 'li_id' }
          });
